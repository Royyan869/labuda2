...
Menemukan koi, mengikuti komunitas, membeli dengan aman

Discovery, trust, commerce, auction

Collector

Mencari ikan bernilai tinggi dan provenance yang jelas

Structured content, seller reputation, transaction protection

Breeder / Farm

Menampilkan koleksi dan menjual ikan secara terstruktur

Store identity, listing, auction, content

Dealer / Seller

Menjual berulang, membangun reputasi, menerima pembayaran terstruktur

Commerce tools, chat negotiation, seller operations

Community Creator

Edukasi, showcase, membangun audience

Content, social graph, share

Admin / Operator

Menjaga trust, dispute, finance, dan platform integrity

Operations console, KYC, moderation, ledger audit
...
Kegagalan atau pembekuan payout tidak menghapus economic ownership seller terhadap saldo ledger.

Payout authority hanya tersedia setelah persyaratan KYC dipenuhi dan disetujui.

Client bukan authority untuk authorization, price, coin cap, order state, atau ledger.

Semua privileged admin action harus tercatat dalam audit trail.
...
6.3 Sharing, Repost & Commerce Reference

Labuda membedakan Internal Share, Social Share/Repost, dan Commerce Response/Reference. Ketiganya adalah intent bisnis yang berbeda dan tidak boleh diperlakukan sebagai sinonim.

Internal Share

Internal Share berarti actor membagikan resource yang sudah ada ke surface internal Labuda. Resource dapat milik actor sendiri atau user lain selama resource tersebut dapat diakses menurut visibility/access rule yang berlaku.

Resource yang dapat di-internal-share:
...
Untuk Content → Feed, Internal Share merupakan bentuk khusus Social Share/Repost: actor membuat Content carrier baru yang mereferensikan Content sumber. Actor/sharer adalah pemilik Content baru; source Content dan source author tetap menjadi sumber referensi/atribusi.

Untuk For Sale/Auction/Profile → Feed, actor membuat Content carrier miliknya sendiri yang membawa typed reference ke resource sumber. Share tersebut tidak memindahkan ownership dan tidak mengubah lifecycle resource sumber.

Untuk Chat, resource yang dibagikan dikirim sebagai reference/attachment pada message. Message sender tidak menjadi owner atau authority atas resource sumber.

Commerce Response / Commerce Reference

Commerce Response adalah intent berbeda dari Internal Share. Actor memasukkan For Sale atau Auction miliknya sendiri ke:
...
Ownership source harus sama dengan actor. Resource milik user lain tidak boleh diperlakukan sebagai Commerce Response; bila ingin membagikan resource milik user lain, gunakan Internal Share.

Pada Create Content, commerce reference menggunakan typed immutable resource occurrence. Pada Comment dan Chat, association disimpan pada record interaksi masing-masing. Surface sosial/interaksi hanya menyimpan reference; authority lifecycle tetap berada pada commerce domain.

Create For Sale from Chat / Comment

Chat dan Comment dapat menjadi originating context untuk membuat For Sale baru, tetapi keduanya tidak memiliki creation authority terpisah.

Chat / Comment
↓
Create For Sale
↓
canonical For Sale creation flow
↓
For Sale successfully created
↓
return to originating Chat / Comment
↓
send/reference newly created For Sale

Create Auction from Chat / Comment bukan flow creation tersendiri. Auction dibuat melalui canonical Auction creation flow. Setelah tersedia, Auction milik actor dapat dipilih dan dikirim/reference melalui Chat atau Comment sesuai interaction capability.
...
Internal Share dan Commerce Response memiliki ownership semantics yang berbeda:

Internal Share: resource existing dapat milik actor atau user lain, selama actor berhak mengaksesnya.

Commerce Response: For Sale/Auction yang dimasukkan ke Create Content, Comment, atau Chat harus milik actor sendiri.

Product = barang/produk dan canonical product identity.
...
Jangan menggunakan attachment, reference, share, dan repost sebagai sinonim. Intent menentukan ownership rule, persistence boundary, dan validation boundary.
...
Receivable/settlement from payment provider

1002

BANK_SETTLEMENT_CASH

Asset

Cash held/settled through designated bank structure

2001

ESCROW_HOLDING_LIABILITY

Liability

Buyer-related transaction obligation while funds are held

2002

SELLER_AVAILABLE_PAYABLE

Liability

Seller proceeds available for payout

2003

COIN_LOYALTY_LIABILITY

Liability

Outstanding coin liability
...
Payment provider fee when contractually borne by platform
...
Platform transaction fee / commission dari marketplace transactions.
...
Marketplace liquidity rendah

High

Supply-first onboarding, community acquisition, auction events

Fraud / seller abuse

High

KYC, trust layers, dispute, payout controls, audit

Live-fish logistics failure

High

Shipping policy, evidence-based DOA workflow, logistics adapters

Payment inconsistency

High

Idempotency, verified webhooks, canonical payment state, reconciliation

Ledger discrepancy

Critical

Double-entry accounting, ACID, reconciliation

Web complexity terlalu besar

Medium

Keep Next.js as public acquisition surface, not feature parity

Infrastructure scaling

Medium

Stateless services where appropriate, PostgreSQL authority, horizontal scale path

Regulatory/payment structure

Critical

Partner/payment architecture review, legal compliance before production settlement model
...
B. Content & Interaction Sharing Contract

B.1 Canonical Intent Boundaries
...
Profile, Content, For Sale, Auction

Feed, Chat

Existing resource; actor or another accessible user
...
Source Content may belong to another user; new carrier Content belongs to actor

Commerce Response / Reference

For Sale, Auction

Create Content, Comment, Chat

Source must belong to actor

Internal Share and Social Share/Repost do not transfer ownership or mutate the source commerce resource. Commerce Response/Reference consumers do not become authority over For Sale/Auction lifecycle.

B.2 Commerce Reference Validation

For Sale/Auction references in Content, Comment, and Chat use the canonical commerce reference validation boundary for resource existence and displayability.

Canonical displayability rules:

For Sale is referenceable only while it is in the active/displayable state.

Auction is referenceable while scheduled or active.

Nonexistent or non-displayable commerce resources are rejected at the reference boundary.

Ownership/capability rules for Commerce Response are separate from displayability validation and belong to the appropriate intent boundary; they must not be duplicated as competing commerce authority inside social consumers.

B.3 Content Resource Occurrence

Create Content may carry a typed resource occurrence for sharing and Commerce Response. The occurrence is an immutable reference to its source resource, not a duplicate commerce authority.
...
direct_commerce_insert_content accepts only For Sale/Auction and represents an own-resource Commerce Response. share_to_feed represents feed sharing/repost behavior.

Mentioned users attached to Content creation use the canonical content_mentioned_users mechanism. Normal Content creation and occurrence-aware Content creation must preserve the same mention validation and persistence semantics; mentions must not be silently dropped when a resource occurrence is present.

B.4 Chat and Comment Commerce Interaction

Chat and Comment are interaction contexts, not commerce creation authorities.

Create For Sale from Chat/Comment: navigate to the canonical For Sale creation flow, then return with the newly created For Sale reference.

Send existing For Sale/Auction in Chat: use the Chat commerce reference/attachment path; commerce ownership remains in the commerce domain.

Send existing For Sale/Auction in Comment: use the Comment Commerce Reference path; commerce ownership remains in the commerce domain.

Create Auction from Chat/Comment: no separate creation flow; use the canonical Auction creation flow first.

Interaction domains must not mutate For Sale/Auction lifecycle or introduce duplicate commerce validation authority.
...
E. Transactional Outbox & Workers

E.1 Outbox Principle
...
Ledger balance must remain mathematically balanced; unexplained discrepancy target = 0

Authorization

Business authorization is server-side

Pricing

Client cannot authoritatively set transaction price

Idempotency

Harmful repeatable mutations are idempotent

Realtime

Realtime signals should be low-latency under defined benchmark conditions

Durability

PostgreSQL is durable business state

Async Reliability

Outbox ensures reliable at-least-once delivery intent

Security

TLS, secret isolation, sensitive-data protection, audit logging

Privacy

KYC/private identity data isolated from public discovery

Observability

Logs, metrics, health checks, worker/payment monitoring

Recovery

Backups and restoration must be operationally tested
...
Coin redemption cap = 20% of product price after seller-funded discount.

Coins do not reduce the shipping, platform commission, or payment gateway fee calculation basis beyond the explicitly defined buyer-cash formula.

Seller-funded discount reduces buyer payment, seller proceeds, and the applicable seller commission basis.

Payment gateway fee is buyer-borne under current policy.

Platform transaction fee is seller-borne under current policy.

Seller withdrawal fee is admin-configurable and may be Rp0.

users.role is canonical as user | admin; seller capability is represented by seller authority/profile.

contents.visibility is the source of truth for content visibility.

Commerce Response in Create Content, Comment, and Chat is restricted to actor-owned For Sale/Auction; existing resources owned by another user are handled as Internal Share, subject to access/visibility rules.

Content creation requires idempotency protection.

Pricing is server-authoritative.

Order/payment state is server-authoritative.

Coin reservation is payment-owned and authoritative through coin_reservations.

Financial mutation uses double-entry ledger semantics.

Redis/WebSocket are not durable transaction state authorities.

Payout authority requires the applicable KYC/trust gate.

Production behavior must converge to one canonical authority; legacy aliases, stale writers, deprecated wrappers, and backward-compatibility residue are not retained merely for historical compatibility.
