...
class _FakeShippingRepository implements ShippingRepository {
  List<ShippingOption> activeOptions = const [];
  ...
  final router = GoRouter(
    ...
    routes: [
      ...
      GoRoute(
        ...
        builder: (context, state) => const ShippingOptionSetupScreen(),
      ),
      GoRoute(
...
void main() {
  group('ShippingOptionSetupScreen canonical UX', () {
            ...
            builder: (context) => Scaffold(
              body: Center(
                child: ElevatedButton(
                  onPressed: () async {
                    await ShippingOptionSetupScreen.open(context);
                  },
    ...
    testWidgets('Settings and selector entry points open the same setup page', (
      tester,
    ) async {
      final repo = _FakeShippingRepository();
      ...
      await tester.pumpWidget(
        _wrapApp(
          ...
          child: Builder(
            builder: (context) => Scaffold(
              body: SellerShippingOptionsSelector(
      ...
      await tester.tap(find.text('Buat opsi pengiriman'));
      await tester.pumpAndSettle();
      expect(find.byType(ShippingOptionSetupScreen), findsOneWidget);
    });
    ...
    testWidgets('HTTP 400 maps raw backend unmarshal error to localized message', (
      ...
      final repo = _FakeShippingRepository()
        ..failCreate = true
        ..customError =
            'json: cannot unmarshal number 50000.0 into Go struct field CreateShippingOptionCoverageRequest.coverages.tariff of type int64';
      await tester.pumpWidget(
        _wrapApp(
          initialLocation: '/',
          repo: repo,
  ...
  group('Settings management list routing', () {
    testWidgets('Settings opens management list not setup', (tester) async {
      ...
      expect(find.byType(ShippingOptionSetupScreen), findsNothing);
    ...
    testWidgets('Tambah Opsi opens canonical setup from list', (tester) async {
      final repo = _FakeShippingRepository();
      await tester.pumpWidget(
        _wrapApp(
          initialLocation: '/settings',
      ...
      // Should open canonical setup page
      expect(find.byType(ShippingOptionSetupScreen), findsOneWidget);
      expect(find.text('Jenis transportasi'), findsOneWidget);
    });
  });
  ...
  group('Listing/Auction direct-add routing', () {
    testWidgets('Create Listing Tambah Opsi opens setup directly', (
      tester,
    ) async {
      final repo = _FakeShippingRepository();
      await tester.pumpWidget(
      ...
      // Tap "Buat opsi pengiriman" (empty-state CTA in selector)
      await tester.tap(find.text('Buat opsi pengiriman'));
      await tester.pumpAndSettle();

      // Should open canonical setup directly, not management list
      expect(find.byType(ShippingOptionSetupScreen), findsOneWidget);
      expect(find.text('Setup Opsi Pengiriman'), findsOneWidget);
    });

    testWidgets('Create Auction selector opens setup directly', (tester) async {
      final repo = _FakeShippingRepository();
      repo.activeOptions = [
        ShippingOption(
          id: 'ship-1',
      ...
      // Tap "Tambah opsi pengiriman" (inline button when options exist)
      await tester.tap(find.text('Tambah opsi pengiriman'));
      await tester.pumpAndSettle();

      // Should open canonical setup directly
      expect(find.byType(ShippingOptionSetupScreen), findsOneWidget);
    });
  });

  group('ShippingOptionSetupScreen edit mode', () {
    testWidgets('edit opens full editor with preloaded fields', (tester) async {
      final repo = _FakeShippingRepository();
      final existingOption = ShippingOption(
        id: 'ship-edit',
        name: 'Bus Kencana',
        type: ShippingType.bus,
          ...
          child: Builder(
            builder: (context) => Scaffold(
              body: Center(
                child: ElevatedButton(
                  onPressed: () async {
                    await ShippingOptionSetupScreen.openEdit(
                      context,
                      existingOption,
                    );
                  },
