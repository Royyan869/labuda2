...
// ============================================================================
// Convenience Providers (Backward Compatibility)
// ============================================================================

/// Future provider untuk single content
final contentFutureProvider = FutureProvider.family<Content?, String>((
  ref,
  contentId,
) async {
  final repo = ref.read(contentRepositoryProvider);
  final result = await repo.getContentById(contentId);
  return result.fold((error) => null, (content) => content);
});

/// Future provider untuk content list
final contentsFutureProvider = FutureProvider.autoDispose<List<Content>>((
  ref,
) async {
  final repo = ref.read(contentRepositoryProvider);
  final result = await repo.getContents();
  return result.fold((error) => [], (contents) => contents);
});

/// Future provider untuk user contents
final userContentsFutureProvider = FutureProvider.family<List<Content>, String>(
  (ref, userId) async {
    final repo = ref.read(contentRepositoryProvider);
    final result = await repo.getContentsByAuthor(userId);
    return result.fold((error) => [], (contents) => contents);
  },
);

/// Future provider untuk trending contents
final trendingContentsFutureProvider =
    FutureProvider.autoDispose<List<Content>>((ref) async {
      final repo = ref.read(contentRepositoryProvider);
      final result = await repo.getTrendingContents();
      return result.fold((error) => [], (contents) => contents);
    });
