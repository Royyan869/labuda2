...
App from zero + belum ada production data = tidak ada alasan backward compatibility, memelihara desain lama, atau menyimpan residu.

Dan konsekuensinya harus keras:

Cleanup belum bisa dilakukan → implementasi belum selesai.
Masih ada consumer legacy → consumer itu bagian dari pekerjaan migrasi dan harus ikut dipindahkan atau dibunuh.
Masih ada duplicate authority → convergence belum selesai.
Masih ada fallback ke desain lama → canonical authority belum benar-benar berdiri sendiri.
Authority belum jelas → audit belum selesai.
Audit belum selesai → implementasi belum boleh dinyatakan selesai.
Implementasi belum selesai → domain tidak boleh ditutup dan tidak boleh dianggap PASS.

Jadi kalimat seperti:

“Legacy masih dipakai, nanti cleanup.”

atau:

“Kita keep dulu untuk compatibility.”

atau:

“Masih ada caller jadi jangan dihapus.”

untuk kondisi Labuda saat ini bukan alasan untuk menunda cleanup. Itu adalah bukti bahwa pekerjaan belum selesai.

Urutannya harus:

tentukan authority → selesaikan replacement end-to-end → pindahkan atau hapus seluruh consumer → buktikan tidak ada authority lama yang tersisa → hard purge → negative proof → baru PASS.

Tidak ada kategori “hijau tapi masih ada residu”.

Dan jika saat audit ditemukan sesuatu yang tidak jelas apakah legacy atau canonical, kamu tidak boleh mengarang solusi atau menghidupkan kembali desain lama. Statusnya harus UNRESOLVED, audit dilanjutkan berlapis sampai authority factual jelas atau hal yang memang merupakan business decision dikembalikan kepada owner.
