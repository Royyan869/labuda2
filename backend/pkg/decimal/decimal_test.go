...
func TestCalculateFinalAmount(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "Order with both discounts",
			base:         100000,
			shipping:     20000,
			platformFee:  2500,
			discount:     5000,
		...
		{
			name:         "Order with excess discount (should not go negative)",
			base:         10000,
			shipping:     5000,
			platformFee:  250,
			discount:     20000,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			base := NewFromFloat(tt.base)
			shipping := NewFromFloat(tt.shipping)
			platformFee := NewFromFloat(tt.platformFee)
			discount := NewFromFloat(tt.discount)
			coinDiscount := NewFromFloat(tt.coinDiscount)

			result := CalculateFinalAmount(base, shipping, platformFee, discount, coinDiscount)
			if result.String() != tt.expected {
				t.Errorf("CalculateFinalAmount() = %s, want %s", result.String(), tt.expected)
			}
		})
	}
}
