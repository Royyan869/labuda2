// Proves that auction winner claim routes (/auctions/:id/claim-token and
// /auctions/:id/claim) reject suspended/banned users via RequireActiveAccount
// middleware.
//
// Pipeline-level tests using the same mock pattern as social_account_gate_test.go.
// Locks the authority contract: claiming an auction (which creates an order and
// initiates payment) requires an active, email-verified account — matching the
// identical constraint already on POST /auctions/:id/bid and POST /orders.
//
// Before this gate was added, a suspended or banned winner could generate a
// pricing token and create an order without being blocked.
package middleware
...
// --- /auctions/:id/claim-token ---

func TestAuctionClaimToken_ActiveUser_Allowed(t *testing.T) {
	...
	handlerCalled := false
	router.POST("/auctions/:id/claim-token", func(c *gin.Context) {
		handlerCalled = true
		c.JSON(http.StatusOK, gin.H{"ok": true})
	})

	req, _ := http.NewRequest("POST", "/auctions/"+uuid.New().String()+"/claim-token", nil)
...
func TestAuctionClaimToken_SuspendedUser_Blocked(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	router := gin.New()
...
func TestAuctionClaimToken_BannedUser_Blocked(t *testing.T) {
	gin.SetMode(gin.TestMode)
	w := httptest.NewRecorder()
	router := gin.New()
