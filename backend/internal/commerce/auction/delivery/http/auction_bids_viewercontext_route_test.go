...
type auctionRouteActorResolver struct {
	actor *capabilityentity.Actor
	err   error
}

func (m *auctionRouteActorResolver) ResolveActor(ctx interface{}, userID uuid.UUID) (*capabilityentity.Actor, error) {
	if m.err != nil {
		return nil, m.err
	}
	if m.actor == nil {
...
func TestAuctionBidsViewerContext_RouteActorChain(t *testing.T) {
	...
	t.Run("anonymous_public_route", func(t *testing.T) {
		...
		router.GET("/api/v1/auctions/:id/bids", func(c *gin.Context) {
			vc = constructAuctionBidsViewerContext(c, nil)
			c.Status(http.StatusOK)
		})

		w := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "/api/v1/auctions/abc/bids", nil)
	...
	cases := []struct {
		...
		},
		{
			...
			actor: &capabilityentity.Actor{
				...
				AccountStatus: "active",
				EmailVerified: true,
				SellerStatus:  auctionStringPtr(string(capabilityentity.SellerStatusActive)),
			},
			wantSeller: true,
		},
		{
			...
			actor: &capabilityentity.Actor{
				...
				AccountStatus: "active",
				EmailVerified: true,
				SellerStatus:  auctionStringPtr(string(capabilityentity.SellerStatusExpired)),
			},
			wantSeller: false,
		},
		{
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			router.Use(middleware.ActorContextInject(&auctionRouteActorResolver{actor: tc.actor}, middleware.ActorContextInjectOptions{}))
	...
	t.Run("actor_resolution_failure_is_safe", func(t *testing.T) {
		...
		router.Use(middleware.ActorContextInject(&auctionRouteActorResolver{err: errors.New("resolver failure")}, middleware.ActorContextInjectOptions{}))
...
func auctionStringPtr(v string) *string {
	return &v
}
